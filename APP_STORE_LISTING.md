# App Store Connect Listing Content for kaira

## App Name

**kaira** (or **kaira - personal astrology** if "kaira" is taken)

## Subtitle (30 characters max)

**your personal astrologer**

## Promotional Text (170 characters max) - Can be updated anytime

Real astrology, calculated from your exact birth chart. Get daily insights based on what's actually happening in the sky right now.

## Description (4000 characters max)

kaira is your personal astrologer—giving you real, personalized astrology based on your exact birth chart and what's actually happening in the sky.

**REAL ASTROLOGY, FOR YOU**
Using your exact birth time and location, kaira calculates your personal birth chart and gives you daily insights based on current planetary transits. This isn't generic sun sign horoscopes—it's astrology tailored specifically to you.

**DAILY INSIGHTS**
Every day, kaira analyzes how current planetary movements are affecting your personal chart and translates that into clear, actionable insights you can actually use. Know when to push forward, when to pause, and what energies are at play in your life.

**UNDERSTAND YOUR CYCLES**
See which astrological cycles are active in your life right now. From Venus transits affecting your relationships to Mars transits energizing your ambitions—kaira breaks down the cosmic weather and what it means for you personally.

**HOROSCOPE SCORES**
Get daily scores for love, career, wellness, and personal growth. See at a glance which areas are flowing and which might need extra attention. Based on actual transit calculations, not randomized content.

**WEEKLY & MONTHLY FORECASTS**
Plan ahead with personalized weekly and monthly forecasts. See what themes are emerging and prepare for the cosmic weather ahead.

**PRIVACY FIRST**
Your birth data stays on your device. No servers, no tracking, no data collection. kaira is astrology that respects your privacy.

**WHAT MAKES KAIRA DIFFERENT**
• Personalized to YOUR birth chart, not just your sun sign
• Based on real astronomical calculations and current transits
• Clear, modern language—no robotic cosmic speak
• Actionable insights you can actually relate to
• Completely offline—your data never leaves your device
• No ads, no subscriptions, no tracking

**FEATURES**
• Full birth chart calculation
• Daily personalized horoscope
• Current astrological cycles
• Daily horoscope scores (love, career, wellness, growth)
• Weekly forecasts
• Monthly forecasts
• Yearly themes
• Transit tracking
• Lunar cycle insights
• Clean, beautiful interface

**PERFECT FOR**
• Anyone curious about astrology beyond sun signs
• People who want personalized daily guidance
• Those interested in understanding cosmic patterns
• Anyone seeking self-reflection tools
• Astrology enthusiasts who value accuracy

kaira combines ancient wisdom with modern calculations to help you understand yourself better and navigate life with more awareness. Your personal astrologer, always in your pocket.

---

**NOTE:** kaira provides astrological interpretations for entertainment and self-reflection purposes. Always use common sense and consult qualified professionals for important life decisions.

## Keywords (100 characters max, separated by commas)

astrology,horoscope,birth chart,zodiac,natal chart,transits,daily horoscope,astrologer,cosmic,planets

## Support URL

https://github.com/jaidev-jayakumar/kairo

## Marketing URL (optional)

https://github.com/jaidev-jayakumar/kairo

## Privacy Policy URL

https://jaidev-jayakumar.github.io/kairo/PRIVACY_POLICY.html

## Category

**Primary:** Lifestyle  
**Secondary:** Entertainment or Health & Fitness

## Age Rating

**4+** (suitable for all ages)

## Copyright

© 2024 Jaidev Jayakumar

---

## Screenshots Required

You'll need screenshots for different iPhone sizes. Apple requires:

- **6.9" Display** (iPhone 16 Pro Max, 15 Pro Max, 14 Pro Max)
- **6.7" Display** (iPhone 16 Plus, 15 Plus, 14 Plus, 13 Pro Max, 12 Pro Max)
- **6.5" Display** (iPhone 11 Pro Max, XS Max)
- **6.1" Display** (iPhone 16 Pro, 16, 15 Pro, 15, 14 Pro, 14, 13 Pro, 13, 12 Pro, 12, 11, XR)
- **5.5" Display** (iPhone 8 Plus, 7 Plus, 6s Plus)

**Minimum:** 3 screenshots, **Maximum:** 10 screenshots per size

### Recommended Screenshots (in order):

1. **Today View** - Showing daily insights and horoscope scores
2. **Cycles View** - Displaying active astrological cycles with progress bars
3. **Birth Chart** - User's personal birth chart
4. **Weekly Forecast** - Weekly insights and themes
5. **Onboarding** - Show how easy it is to get started
6. **Profile/Scores** - Detailed horoscope scores breakdown

### Screenshot Tips:

- Use actual app screenshots (no mockups required)
- Can add text overlays to highlight features
- Keep text minimal and readable
- Show the app's best features first
- Consider using a tool like "Screenshot Creator" or "App Store Screenshot Generator"

---

## What's New Text (for future updates)

```
Version 1.0 - Initial Release

Welcome to kaira - your personal astrologer!

• Real astrology calculated from your exact birth chart
• Daily insights based on current planetary transits
• Horoscope scores for love, career, wellness, and growth
• Active cycle tracking
• Weekly and monthly forecasts
• Privacy-first: your data stays on your device
• No ads, no tracking, completely offline

We'd love to hear your feedback! Please rate kaira if you enjoy using it.
```

---

## Hosting Your Privacy Policy

You need to host PRIVACY_POLICY.md online. Easiest options:

### Option 1: GitHub Pages (Free, Recommended)

1. Go to your repo settings
2. Enable GitHub Pages
3. Set source to `main` branch
4. Your privacy policy will be at: `https://[your-username].github.io/kairo/PRIVACY_POLICY.html`

### Option 2: Convert to HTML and host anywhere

```html
<!DOCTYPE html>
<html>
  <head>
    <title>kaira Privacy Policy</title>
    <meta charset="utf-8" />
    <style>
      body {
        max-width: 800px;
        margin: 40px auto;
        padding: 20px;
        font-family: -apple-system, sans-serif;
      }
      h1 {
        color: #ff6b6b;
      }
      h2 {
        margin-top: 30px;
      }
    </style>
  </head>
  <body>
    <!-- Paste converted markdown here -->
  </body>
</html>
```

### Option 3: Use a free hosting service

- **Vercel:** Fast, free, connects to GitHub
- **Netlify:** Similar to Vercel
- **Render:** Another good option

---

## Before Submission Checklist

✅ App icon added (1024x1024px)  
✅ Screenshots created (at least 3 per required size)  
✅ Privacy Policy hosted and URL added  
✅ Terms of Service hosted (optional but recommended)  
✅ Support email/URL configured  
✅ App tested on real device  
✅ All text content reviewed for typos  
✅ Version 1.0, Build 1 set correctly  
✅ Bundle identifier matches: com.github.jaidev-jayakumar.kairo

---

## Pricing

**Free** (for v1 - you can add IAP/subscriptions later)

## Availability

**All territories** (unless you have specific restrictions)
